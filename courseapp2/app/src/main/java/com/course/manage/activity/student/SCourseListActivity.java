...
public class SCourseListActivity extends AppCompatActivity implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bgaRefreshLayoutAdapter = new BGARecyclerViewAdapter<Course>(recyclerView, R.layout.item_student_course_all) {
            @Override
            protected void fillData(BGAViewHolderHelper helper, int position, Course model) {
                ...
                if (model.isShowTime()){
                    ...
                    Glide.with(SCourseListActivity.this).load(R.drawable.baseline_keyboard_arrow_up_24).into(helper.getImageView(R.id.iv_arrow_open));
                }else{
                    helper.getView(R.id.ll_cours_time_root).setVisibility(View.GONE);
                    Glide.with(SCourseListActivity.this).load(R.drawable.baseline_keyboard_arrow_down_24).into(helper.getImageView(R.id.iv_arrow_open));
                }
                ...
                if (model.getSelected()!=1){
                    helper.getTextView(R.id.tv_select_course).setVisibility(TextUtils.equals(model.getStatus(),"select start")?View.VISIBLE:View.GONE);
                }else{
                    helper.getTextView(R.id.tv_select_course).setVisibility(View.GONE);
                }
                helper.getTextView(R.id.tv_select_course).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        AlertDialog.Builder builder = new AlertDialog.Builder(SCourseListActivity.this);
                        builder.setTitle("Select It?")
                                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        dialog.dismiss();
                                        selectCourse(model.getId());
                                    }
                                })
    ...
    public void selectCourse(int courseId){
        RequestParams requestParams = new RequestParams(HttpUtils.studentSelectCourse+"?token="+App.token);
    ...
    public void onBGARefreshLayoutBeginRefreshing(BGARefreshLayout refreshLayout) {
        RequestParams requestParams = new RequestParams(HttpUtils.studentCourseAlllist+"?token="+ App.token);
        HttpUtils.post(requestParams, new HttpUtils.HttpCallack() {
            @Override
            public void onSuccess(String result) throws JSONException {
                ...
                if (data!=null&&data.length()>0){
                    ...
                    for (int i = 0; i < data.length(); i++) {
                        ...
                        course.setSelected(item.optInt("selected"));
